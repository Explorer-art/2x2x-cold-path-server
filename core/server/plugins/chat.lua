...
local function get_prefix(group)
    ...
    if groups[group].prefix ~= false then
        return "<color="..groups[group].prefix_color..">"..groups[group].prefix.."</color> "
    else
        return ""
    end
end
...
local function get_suffix(group, name)
    ...
    if groups[group].suffix ~= false then
        return "<color="..groups[group].suffix_color..">"..groups[group].suffix.."</color>"
    else
        return ""
    end
end

local function get_level_prefix(name)
	local prefix

	if db.levels[name].level >= 10 then
	    prefix = "<color=#8b00ff>[И]</color> "
	elseif db.levels[name].level >= 8 then
...
local function attribute_message(text,client)
    ...
    if client_data.group == "operator" or client_data.group == "owner" or client_data.group == "senior_admin" or client_data.group == "prem" and db.players_data[client_data.name].invise == false then
    ...
    if client_data.group == "default" or db.players_data[client_data.name].invise == true then
    	if db.levels[client_data.name] then
    		prefix = get_level_prefix(client_data.name)
	    end
    end
...
function M.on_player_joined(client)
    ...
    if client_data.group == "operator" or client_data.group == "owner" or client_data.group == "senior_admin" or client_data.group == "prem" and db.players_data[client_name].invise == false then
    	client_name = "<color=yellow>"..client_name.."</color>"
    end
